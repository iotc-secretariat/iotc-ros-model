...
UPDATE ros_common.trip_vessel t SET fish_storage_capacity_value = m.value, fish_storage_capacity_unit = lower(m.unit) FROM ros_common.capacities m WHERE t.fish_storage_capacity_id = m.id;
...
UPDATE ros_ps.gear_specifications t SET maximum_net_depth_value = m.value, maximum_net_depth_unit = lower(m.unit) FROM ros_common.depths m WHERE t.maximum_net_depth_id = m.id;
...
UPDATE ros_pl.bait_fishing_operations t SET event_depth_value = m.value, event_depth_unit = lower(m.unit) FROM ros_common.depths m WHERE t.event_depth_id = m.id;
...
UPDATE ros_gn.gillnet_configuration t SET net_depth_value = m.value, net_depth_unit = lower(m.unit) FROM ros_common.depths m WHERE t.net_depth_id = m.id;
...
UPDATE ros_ll.branchline_sections t SET diameter_value = m.value, diameter_unit = lower(m.unit) FROM ros_common.diameters m WHERE t.diameter_id = m.id;
...
UPDATE ros_ll.mitigation_measures t SET hook_sinker_distance_value = m.value, hook_sinker_distance_unit = lower(m.unit) FROM ros_common.distances m WHERE t.hook_sinker_distance_id = m.id;
...
UPDATE ros_ll.tori_line_details t SET streamer_distance_value = m.value, streamer_distance_unit = lower(m.unit) FROM ros_common.distances m WHERE t.streamer_distance_id = m.id;
...
UPDATE ros_gn.gillnet_configuration t SET distance_between_floats_value = m.value, distance_between_floats_unit = lower(m.unit) FROM ros_common.distances m WHERE t.distance_between_floats_id = m.id;
...
UPDATE ros_common.trip_vessel_main_engines t SET main_engines_make = m.make, main_engines_value = m.value, main_engines_unit = lower(m.unit) FROM ros_common.engines m WHERE t.main_engines_id = m.id;
...
ALTER TABLE ros_common.biometric_information ADD column estimated_weight_method_code char(2);
UPDATE ros_common.biometric_information t SET estimated_weight_value = m.value, estimated_weight_unit = lower(m.unit), estimated_weight_type_of_measurement_code = m.type_of_measurement_code, estimated_weight_processing_type_code = m.processing_type_code, estimated_weight_method_code = m.weight_estimation_method_code FROM ros_common.estimated_weights m WHERE t.estimated_weight_id = m.id;
...
ALTER TABLE ros_ps.catch_details ADD column estimated_weight_method_code char(2);
...
ALTER TABLE ros_ll.catch_details ADD column estimated_weight_method_code char(2);
...
ALTER TABLE ros_pl.catch_details ADD column estimated_weight_method_code char(2);
...
ALTER TABLE ros_gn.catch_details ADD column estimated_weight_method_code char(2);
...
UPDATE ros_ll.tori_line_details t SET attached_height_value = m.value, attached_height_unit = lower(m.unit) FROM ros_common.heights m WHERE t.attached_height_id = m.id;
...
UPDATE ros_common.trip_vessel t SET loa_value = m.value, loa_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.loa_id = m.id;
...
UPDATE ros_ps.gear_specifications t SET maximum_net_length_value = m.value, maximum_net_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.maximum_net_length_id = m.id;
...
UPDATE ros_ll.branchline_sections t SET length_value = m.value, length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.length_id = m.id;
...
UPDATE ros_ll.floatlines t SET floatline_length_value = m.value, floatline_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.floatline_length_id = m.id;
...
UPDATE ros_ll.leader_set t SET total_branchline_minimum_length_value = m.value, total_branchline_minimum_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.total_branchline_minimum_length_id = m.id;
...
UPDATE ros_ll.leader_set t SET total_branchline_maximum_length_value = m.value, total_branchline_maximum_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.total_branchline_maximum_length_id = m.id;
...
UPDATE ros_ll.setting_operations t SET mainline_set_length_value = m.value, mainline_set_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.mainline_set_length_id = m.id;
...
UPDATE ros_ll.tori_line_details t SET tori_line_length_value = m.value, tori_line_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.tori_line_length_id = m.id;
...
UPDATE ros_ll.tori_line_details t SET streamer_line_length_max_value = m.value, streamer_line_length_max_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.streamer_line_length_max_id = m.id;
...
UPDATE ros_ll.tori_line_details t SET streamer_line_length_min_value = m.value, streamer_line_length_min_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.streamer_line_length_min_id = m.id;
...
UPDATE ros_gn.gillnet_configuration t SET droplines_length_value = m.value, droplines_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.droplines_length_id = m.id;
...
UPDATE ros_gn.gillnet_configuration t SET net_length_value = m.value, net_length_unit = lower(m.unit) FROM ros_common.lengths m WHERE t.net_length_id = m.id;
...
UPDATE ros_common.trip_vessel t SET autonomy_range_value = m.value, autonomy_range_unit = lower(m.unit) FROM ros_common.ranges m WHERE t.autonomy_range_id = m.id;
...
UPDATE ros_ps.gear_specifications t SET bunt_stretched_mesh_size_value = m.value, bunt_stretched_mesh_size_unit = lower(m.unit) FROM ros_common.sizes m WHERE t.bunt_stretched_mesh_size_id = m.id;
...
UPDATE ros_ps.gear_specifications t SET mid_net_stretched_mesh_size_value = m.value, mid_net_stretched_mesh_size_unit = lower(m.unit) FROM ros_common.sizes m WHERE t.mid_net_stretched_mesh_size_id = m.id;
...
UPDATE ros_gn.gillnet_configuration_stretched_mesh_sizes t SET stretched_mesh_size_value = m.value, stretched_mesh_size_unit = lower(m.unit) FROM ros_common.sizes m WHERE t.stretched_mesh_size_id = m.id;
...
UPDATE ros_ll.setting_operations t SET vessel_speed_value = m.value, vessel_speed_unit = lower(m.unit) FROM ros_common.speeds m WHERE t.vessel_speed_id = m.id;
...
UPDATE ros_ll.setting_operations t SET line_setter_speed_value = m.value, line_setter_speed_unit = lower(m.unit) FROM ros_common.speeds m WHERE t.line_setter_speed_id = m.id;
...
UPDATE ros_ll.additional_catch_details_on_ssi t SET leader_thickness_value = m.value, leader_thickness_unit = lower(m.unit) FROM ros_common.thicknesses m WHERE t.leader_thickness_id = m.id;
...
UPDATE ros_common.trip_vessel t SET tonnage_value = m.value, tonnage_unit = lower(m.unit) FROM ros_common.tonnages m WHERE t.tonnage_id = m.id;
...
UPDATE ros_ll.mitigation_measures t SET average_sinker_weight_value = m.value, average_sinker_weight_unit = lower(m.unit) FROM ros_common.weights m WHERE t.average_sinker_weight_id = m.id;
...
UPDATE ros_gn.sinkers_by_type t SET average_sinker_weight_value = m.value, average_sinker_weight_unit = lower(m.unit) FROM ros_common.weights m WHERE t.average_sinker_weight_id = m.id;
