...
SELECT od.id                                                                                                                                       AS trip_id,
    ...
    GROUP BY od.id, od.uid, (date_part('year'::text, s.start_setting_date_and_time)), (date_part('month'::text, s.start_setting_date_and_time)), (ros_meta.to_grid_1(s.start_setting_latitude, s.start_setting_longitude)),
...
SELECT od.id                                                                                                                                       AS trip_id,
    ...
    GROUP BY od.id, od.uid, (date_part('year'::text, s.start_setting_date_and_time)), (date_part('month'::text, s.start_setting_date_and_time)), (ros_meta.to_grid_1(s.start_setting_latitude, s.end_setting_latitude)),
...
SELECT ps_o_d.source,
    ...
    FROM ros_ps.observer_data ps_o_d
             JOIN ros_common.trip t ON ps_o_d.trip_id = t.id
...
SELECT pl_o_d.source,
    ...
    FROM ros_pl.observer_data pl_o_d
             JOIN ros_common.trip t ON pl_o_d.trip_id = t.id
             LEFT JOIN ros_common.trip_vessel gvt ON t.id = gvt.trip_id
             LEFT JOIN ros_common.trip_observer otd ON t.id = otd.trip_id
             LEFT JOIN ros_common.observer oi ON otd.observer_id = oi.contact_id
             LEFT JOIN ros_common.vessel vi ON gvt.vessel_id = vi.id
...
SELECT ll_o_d.source,
    ...
    FROM ros_ll.observer_data ll_o_d
            JOIN ros_common.trip t ON ll_o_d.trip_id = t.id
             LEFT JOIN ros_common.trip_vessel gvt ON t.id = gvt.trip_id
             LEFT JOIN ros_common.trip_observer otd ON t.id = otd.trip_id
             LEFT JOIN ros_common.observer oi ON otd.observer_id = oi.contact_id
             LEFT JOIN ros_common.vessel vi ON gvt.vessel_id = vi.id
...
SELECT gn_o_d.source,
    ...
    FROM ros_gn.observer_data gn_o_d
             JOIN ros_common.trip t ON gn_o_d.trip_id = t.id
             LEFT JOIN ros_common.trip_vessel gvt ON t.id = gvt.trip_id
             LEFT JOIN ros_common.trip_observer otd ON t.id = otd.trip_id
             LEFT JOIN ros_common.observer oi ON otd.observer_id = oi.contact_id
             LEFT JOIN ros_common.vessel vi ON gvt.vessel_id = vi.id
...
WITH ps AS (SELECT date_part('year'::text, s.start_setting_date_and_time)                                                                                      AS year,
                ...
                FROM ros_ps.observer_data od
                         JOIN ros_common.trip t ON od.trip_id = t.id
